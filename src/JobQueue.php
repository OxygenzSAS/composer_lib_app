...
class JobQueue {
	 ...
	 * @return self
	 */
	public function selectPipeline(string $pipeline): self {
		$this->pipeline = $pipeline;
	...
	protected function runPreChecks() {
		...
		if($this->isMysqlQueueType() || $this->isSqliteQueueType()) {
			$this->checkAndIfNecessaryCreateselfTable();
		}
	...
	*	@param bool $split
	...
	protected function checkAndIfNecessaryCreateselfTable(): void {
		$cache =& self::$cache;
